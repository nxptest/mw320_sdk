...
************************************************************************************/
  ...
  #define mRamStartAddress_c (0x04000000)
  #define mRamEndAddress_c   (0x04016000)

  #define mRamStartAddress2_c (0x04020000)
  #define mRamEndAddress2_c   (0x04030000)

#else

  #define mRamStartAddress_c   (uint32_t)(0x20000000)
  #define mRamEndAddress_c     (uint32_t)(0x2001FFFF)

#endif

#define isWithin(addr, len, start, end)        ((!(start)) ? true : (((uint32_t)(addr)> (uint32_t)(start)) && ((uint32_t)(addr + len - 1) < (uint32_t)(end))))

/* Flash memory limits */
#define mFlashStartAddress_c (0x1F000000)
#define mFlashEndAddress_c   (0x1FFFFFFF)
...
void FSCI_Error(uint8_t errorCode, uint32_t fsciInterface)
{
    ...
    if( !mFsciErrorReported )
    {
        ...
        mFsciErrorMsg.checksum = FSCI_computeChecksum((const void*)&mFsciErrorMsg.header.opGroup, size-2 );
...
#ifdef CPU_JN518X
    if( isWithin(addr, len, mRamStartAddress_c, mRamEndAddress_c) ||
        isWithin(addr, len, mRamStartAddress2_c, mRamEndAddress2_c) )
#else
...
bool_t FSCI_ReadMemoryBlock(clientPacket_t * pData, uint32_t fsciInterface)
{
...
#ifdef CPU_JN518X
    else if ((isWithin(addr, len, mRamStartAddress_c, mRamEndAddress_c) ||
        isWithin(addr, len, mRamStartAddress2_c, mRamEndAddress2_c) ||
        isWithin(addr, len, mFlashStartAddress_c, mFlashEndAddress_c)))
#else
    ...
    {
#ifdef CPU_JN518X
        if (isWithin(addr, len, mFlashStartAddress_c, mFlashEndAddress_c) &&
            FLASH_VERIFY_ERASE((uint32_t)addr, len) == kStatus_FLASH_Success)
        {
            FLib_MemSet(pPkt->structured.payload, 0, len);
        }
        else
#endif
...
bool_t FSCI_GetNumberOfFreeBuffersFunc(clientPacket_t * pData, uint32_t fsciInterface)
{
    ...
    FSCI_transmitPayload( gFSCI_CnfOpcodeGroup_c, mFsciMsgGetNumberOfFreeBuffersReq_c, (void*)&gFreeMessagesCount, (uint16_t)sizeof(gFreeMessagesCount), fsciInterface);
    return FALSE;
}
...
bool_t FSCI_MsgGetWakeUpReasonReqFunc(clientPacket_t * pData, uint32_t fsciInterface)
{
    MEM_BufferFree(pData);
#if !defined(CPU_JN518X)
    FSCI_transmitPayload( gFSCI_CnfOpcodeGroup_c, mFsciMsgGetWakeUpReasonReq_c, (const uint8_t *)&PWRLib_MCU_WakeupReason, (uint16_t)sizeof(PWRLib_WakeupReason_t), fsciInterface);
#else
    PWR_WakeupReason_t wakeupReason = PWR_GetWakeupReason();
    FSCI_transmitPayload( gFSCI_CnfOpcodeGroup_c, mFsciMsgGetWakeUpReasonReq_c, (const uint8_t *)&wakeupReason, (uint16_t)sizeof(PWR_WakeupReason_t), fsciInterface);
#endif
    return FALSE;
}
...
#endif /* gFSCI_BootloaderCommands_c */
