...
{
    FRM_OSA_MutexLock(mFsciCommData[fsciInstance].syncHostMutexId, osaWaitForever_c);
...
void FSCI_HostSyncUnlock(uint32_t fsciInstance)
{
    ...
    pFsciHostSyncRsp = NULL;
    FRM_OSA_MutexUnlock(mFsciCommData[fsciInstance].syncHostMutexId);
}
#endif
...
void FSCI_commInit(const gFsciSerialConfig_t* pSerCfg )
{
    ...
    if( (gFsciHostSyncRspEventId = FRM_OSA_EventCreate(TRUE)) == NULL )
    {
        panic( ID_PANIC(0,0), (uint32_t)FSCI_commInit, 0, 0 );
        return;
    }
...
#if gFsciHostSupport_c
            if( (mFsciCommData[i].syncHostMutexId = FRM_OSA_MutexCreate()) == NULL )
            {
                panic( ID_PANIC(0,0), (uint32_t)FSCI_commInit, 0, 0 );
                break;        
            }
...
#if gFsciRxAck_c
            if( (mFsciCommData[i].syncTxRxAckMutexId = FRM_OSA_MutexCreate()) == NULL )
            {
                panic( ID_PANIC(0,0), (uint32_t)FSCI_commInit, 0, 0 );
                break;        
            }
...
void FSCI_receivePacket( void* param )
{
    ...
    while( readBytes )
    {
        ...
        else
        {
            ...
            {
                pCommData->pPacketFromClient = MEM_BufferAlloc( sizeof(clientPacketHdr_t) + pCommData->pktHeader.len + gFsci_TailBytes_c );
                if( NULL != pCommData->pPacketFromClient )
                {
...
#if gFsciHostSyncUseEvent_c
                        FRM_OSA_EventSet(gFsciHostSyncRspEventId, gFSCIHost_RspReady_c);
#endif
                    }
                    else
#endif                  
...
void FSCI_transmitPayload( uint8_t OG, uint8_t OC, const uint8_t *pMsg, uint16_t msgLen, uint32_t fsciInterface )
{
    ...
    checksum = FSCI_computeChecksum((const uint8_t*)&header + 1, sizeof(header) - 1);
    checksum ^= FSCI_computeChecksum((const uint8_t*)pMsg, msgLen);
    if( virtInterface )
    {
...
uint8_t* FSCI_GetFormattedPacket(uint8_t OG, uint8_t OC, void *pMsg, uint16_t msgLen, uint16_t *pOutLen)
{
    ...
    if ((msgLen <= gFsciMaxPayloadLen_c) && (pOutLen) && (pMsg))
    {
...
static void FSCI_SendPacketToSerialManager(uint32_t fsciInterface, uint8_t *pPacket, uint16_t packetLen)
{
    ...
    FRM_OSA_MutexLock(pCommData->syncTxRxAckMutexId, osaWaitForever_c);
    ...
    FRM_OSA_MutexUnlock(pCommData->syncTxRxAckMutexId);
#else /* gFsciRxAck_c */
