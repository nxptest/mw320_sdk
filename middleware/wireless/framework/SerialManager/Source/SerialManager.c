...
#else
#if USE_SDK_OSA
FRM_OSA_TASK_DEFINE( SerialManagerTask, gSerialTaskPriority_c, 1, gSerialTaskStackSize_c, FALSE );
...
void Serial_InitManager( void )
{
    ...
    if( FALSE == initialized )
    {
...
#else
#if USE_SDK_OSA
        mSMTaskEventId = FRM_OSA_EventCreate( TRUE );
        ...
        {
            gSerialManagerTaskId = FRM_OSA_TaskCreate(FRM_OSA_TASK(SerialManagerTask), NULL);
...
#endif
{
    ...
    while( 1 )
    {
        ...
        (void)FRM_OSA_EventWait(mSMTaskEventId, osaEventFlagsAll_c, FALSE, osaWaitForever_c ,&mSMTaskEventFlags);
#endif
#endif
...
//                mDrvData[i].uartState.pRxData = &pSer->rxBuffer[pSer->rxIn];
            ...
            if( (status == gSerial_Success_c) &&
                ((pSer->txSyncSemId = FRM_OSA_SemaphoreCreate(0)) == NULL) )
            {
...
#if gSerialMgr_BlockSenderOnQueueFull_c
            if( (status == gSerial_Success_c) &&
                ((pSer->txQueueSemId = FRM_OSA_SemaphoreCreate( 0)) == NULL) )
            {
                status = gSerial_SemCreateError_c;
            }
...
#if gSMGR_UseOsSemForSynchronization_c
            FRM_OSA_SemaphoreDestroy(pSer->txSyncSemId);
#if gSerialMgr_BlockSenderOnQueueFull_c
            FRM_OSA_SemaphoreDestroy(pSer->txQueueSemId);
#endif /* gSerialMgr_BlockSenderOnQueueFull_c */
#endif /* gSMGR_UseOsSemForSynchronization_c */
    } while (0);
    FRM_OSA_InterruptEnable();
                                  ...
                                  void *pTxParam )
{
...
#endif
    {
        ...
        osaTaskId_t taskHandler = FRM_OSA_TaskGetId();
#endif
        ...
        do {
            ...
            else
            {
                ...
                if(taskHandler != gSerialManagerTaskId)
                {
                    (void)FRM_OSA_SemaphoreWait(pSer->txQueueSemId, osaWaitForever_c);
                }
                else
#endif
                {
                                 ...
                                 uint16_t bufLen )
{
    ...
    if( FRM_OSA_TaskGetId() != gSerialManagerTaskId )
        ...
        (gSerial_OutOfMemory_c != status) )
    {
        ...
        if( cb )
        {
            (void)FRM_OSA_SemaphoreWait(pSer->txSyncSemId, osaWaitForever_c);
        }
        else
#endif
        {
...
//        mSerial_IncIdx_d(pSer->txCurrent, gSerialMgrTxQueueSize_c)
        pSer->state = 0;
#if USE_SDK_OSA
        (void)FRM_OSA_EventSet(mSMTaskEventId, gSMGR_TxNew_c);
#endif
    }
...
#if USE_SDK_OSA
    (void)FRM_OSA_EventSet(mSMTaskEventId, gSMGR_Rx_c);
#endif
}
#endif
...
#if USE_SDK_OSA
    (void)FRM_OSA_EventSet(mSMTaskEventId, gSMGR_TxDone_c);
#endif
    }
}
    ...
    if( FALSE == pSer->txMntOngoing )
    {
        ...
        while( pSer->txQueue[pSer->txOut].dataSize == 0 )
        {
            ...
            if( pSer->txBlockedTasks )
            {
                pSer->txBlockedTasks--;
                FRM_OSA_InterruptEnable();
                (void)FRM_OSA_SemaphorePost(pSer->txQueueSemId);
            }
            else
            {
              FRM_OSA_InterruptEnable();
...
static void Serial_SyncTxCallback(void *pSer)
{
    (void)FRM_OSA_SemaphorePost(((serial_t *)pSer)->txSyncSemId );
}
#endif
