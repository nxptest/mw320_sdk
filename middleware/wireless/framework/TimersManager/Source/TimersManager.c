 ...
 */
FRM_OSA_TASK_DEFINE( TMR_Task, gTmrTaskPriority_c, 1, gTmrTaskStackSize_c, FALSE );
...
void TMR_Init(void)
{
    ...
    if( !mInitialized )
    {
       mInitialized = TRUE;
        ...
        mTimerThreadEventId = FRM_OSA_EventCreate(TRUE);
        ...
        {
            timerThreadId = FRM_OSA_TaskCreate(FRM_OSA_TASK(TMR_Task), NULL);
...
void StackTimer_ISR(void)
{
    ...
    ev |= mTmrTimerEvent_c;
#endif
    (void)FRM_OSA_EventSet(mTimerThreadEventId, ev);
...
)
{
    ...
    while(1)
    {
        (void)FRM_OSA_EventWait(mTimerThreadEventId, osaEventFlagsAll_c, FALSE, osaWaitForever_c, &ev);
#endif
...
void TMR_EnableTimer(tmrTimerID_t tmrID)
{
    ...
    if (signalTask == TRUE)
    {
        (void)FRM_OSA_EventSet(mTimerThreadEventId, mTmrTimerEvent_c);
    }
}
...
void TMR_RTCInit(void)
{
    ...
    if( !gRTCInitFlag )
    {
        ...
        /* Overwrite old ISR */
        FRM_OSA_InstallIntHandler(RTC_IRQn, TMR_RTCAlarmNotify);
...
void TMR_PITInit(void)
{
    ...
    if( !gPITInitFlag )
    {
        ...
        /* Overwrite old ISR */
        FRM_OSA_InstallIntHandler(pitIrqs[0][gTmrPitInstance_c], TMR_PIT_ISR);
