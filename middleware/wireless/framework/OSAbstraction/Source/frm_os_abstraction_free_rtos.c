...
#include "frm_os_abstraction_free_rtos.h"
#include <string.h>
...
#endif
#ifndef FSL_FRM_OSA_MAIN_FUNC_DISABLE
...
#if OsaBasePri_d
static uint32_t g_base_priority_array[FRM_OSA_MAX_ISR_CRITICAL_SECTION_DEPTH];
...
/*FUNCTION**********************************************************************
 *
 * Function Name : FRM_OSA_TaskGetPriority
 * Description   : This function returns task's priority by task handler.
 *
 *END**************************************************************************/
osaTaskPriority_t FRM_OSA_TaskGetPriority(osaTaskId_t taskId)
{
  return (osaTaskPriority_t)(PRIORITY_RTOS_TO_FRM_OSA(uxTaskPriorityGet((task_handler_t)taskId)));
}

/*FUNCTION**********************************************************************
 *
 * Function Name : FRM_OSA_TaskSetPriority
 * Description   : This function sets task's priority by task handler.
 *
 *END**************************************************************************/
osaStatus_t FRM_OSA_TaskSetPriority(osaTaskId_t taskId, osaTaskPriority_t taskPriority)
{
    vTaskPrioritySet((task_handler_t)taskId, PRIORITY_FRM_OSA_TO_RTOS(taskPriority));
    return osaStatus_Success;
}

/*FUNCTION**********************************************************************
*
* Function Name : FRM_OSA_TaskCreate
...
*
*END**************************************************************************/
osaTaskId_t FRM_OSA_TaskCreate(osaThreadDef_t *thread_def,osaTaskParam_t task_param)
{
  ...
  if (xTaskCreate(
                  ...
                  PRIORITY_FRM_OSA_TO_RTOS(thread_def->tpriority),  /* initial priority */
...
/*FUNCTION**********************************************************************
 *
 * Function Name : FRM_OSA_TaskDestroy
 ...
 *
 *END**************************************************************************/
osaStatus_t FRM_OSA_TaskDestroy(osaTaskId_t taskId)
{
  osaStatus_t status;
  uint16_t oldPriority;
  /*Change priority to avoid context switches*/
  oldPriority = FRM_OSA_TaskGetPriority(FRM_OSA_TaskGetId());
  (void)FRM_OSA_TaskSetPriority(FRM_OSA_TaskGetId(), FRM_OSA_PRIORITY_REAL_TIME);
...
#endif
  (void)FRM_OSA_TaskSetPriority(FRM_OSA_TaskGetId(), oldPriority);
...
/*FUNCTION**********************************************************************
 *
 * Function Name : FRM_OSA_GetTickCount
 * Description   : This function gets number of ticks.
 *
 *END**************************************************************************/
uint32_t FRM_OSA_GetTickCount(void)
{
    portTickType ticks;
...
/*FUNCTION**********************************************************************
 *
 * Function Name : FRM_OSA_SemaphoreDestroy
 ...
 *
 *END**************************************************************************/
osaStatus_t FRM_OSA_SemaphoreDestroy(osaSemaphoreId_t semId)
{
#if osNumberOfSemaphores
...
/*FUNCTION**********************************************************************
 *
 * Function Name : FRM_OSA_SemaphoreWait
 ...
 *
 *END**************************************************************************/
osaStatus_t FRM_OSA_SemaphoreWait(osaSemaphoreId_t semId, uint32_t millisec)
{
#if osNumberOfSemaphores
...
/*FUNCTION**********************************************************************
 *
 * Function Name : FRM_OSA_SemaphorePost
 ...
 *
 *END**************************************************************************/
osaStatus_t FRM_OSA_SemaphorePost(osaSemaphoreId_t semId)
{
#if osNumberOfSemaphores
...
/*FUNCTION**********************************************************************
 *
 * Function Name : FRM_OSA_MutexLock
 ...
 *
 *END**************************************************************************/
osaStatus_t FRM_OSA_MutexLock(osaMutexId_t mutexId, uint32_t millisec)
{
#if osNumberOfMutexes    
    uint32_t timeoutTicks;
...
/*FUNCTION**********************************************************************
 *
 * Function Name : FRM_OSA_MutexUnlock
 * Description   : This function is used to unlock a mutex.
 *
 *END**************************************************************************/
osaStatus_t FRM_OSA_MutexUnlock(osaMutexId_t mutexId)
{
#if osNumberOfMutexes  
  mutex_t mutex = (mutex_t)mutexId;
  if(mutexId == NULL)
...
/*FUNCTION**********************************************************************
 *
 * Function Name : FRM_OSA_MutexDestroy
 ...
 *
 *END**************************************************************************/
osaStatus_t FRM_OSA_MutexDestroy(osaMutexId_t mutexId)
{
#if osNumberOfMutexes    
  mutex_t mutex = (mutex_t)mutexId;
  if(mutexId == NULL)
...
/*FUNCTION**********************************************************************
 *
 * Function Name : FRM_OSA_EventSet
 ...
 *
 *END**************************************************************************/
osaStatus_t FRM_OSA_EventSet(osaEventId_t eventId, osaEventFlags_t flagsToSet)
{
#if osNumberOfEvents    
  osEventStruct_t* pEventStruct; 
  portBASE_TYPE taskToWake = pdFALSE;
  ...
  if (__get_IPSR())
  {
    ...
    {
      panic(0,(uint32_t)FRM_OSA_EventSet,0,0);
      return osaStatus_Error;
    }
    if (pdTRUE == taskToWake)
    {
...
/*FUNCTION**********************************************************************
 *
 * Function Name : FRM_OSA_EventClear
 ...
 *
 *END**************************************************************************/
osaStatus_t FRM_OSA_EventClear(osaEventId_t eventId, osaEventFlags_t flagsToClear)
{
#if osNumberOfEvents      
...
/*FUNCTION**********************************************************************
 *
 * Function Name : FRM_OSA_EventWait
 ...
 *
 *END**************************************************************************/
osaStatus_t FRM_OSA_EventWait(osaEventId_t eventId, osaEventFlags_t flagsToWait, bool_t waitAll, uint32_t millisec, osaEventFlags_t *pSetFlags)
{
#if osNumberOfEvents  
  osEventStruct_t* pEventStruct; 
...
/*FUNCTION**********************************************************************
 *
 * Function Name : FRM_OSA_EventDestroy
 ...
 *
 *END**************************************************************************/
osaStatus_t FRM_OSA_EventDestroy(osaEventId_t eventId)
{
#if osNumberOfEvents    
  osEventStruct_t* pEventStruct; 
  if(osObjectIsAllocated(&osEventInfo, eventId) == FALSE)
...
/*FUNCTION**********************************************************************
 *
 * Function Name : FRM_OSA_MsgQCreate
 ...
 *
 *END**************************************************************************/
osaMsgQId_t FRM_OSA_MsgQCreate( uint32_t  msgNo )
{
 ...
 *
 *END**************************************************************************/
osaStatus_t FRM_OSA_MsgQGet(osaMsgQId_t msgQId, void *pMessage, uint32_t millisec)
{
#if osNumberOfMessageQs  
  osaStatus_t osaStatus;
  msg_queue_handler_t handler;
...
/*FUNCTION**********************************************************************
 *
 * Function Name : FRM_OSA_MsgQDestroy
 ...
 *
 *END**************************************************************************/
osaStatus_t FRM_OSA_MsgQDestroy(osaMsgQId_t msgQId)
{
#if osNumberOfMessageQs  
  msg_queue_handler_t handler;
  if(msgQId == NULL )
...
/*FUNCTION**********************************************************************
 *
 * Function Name : FRM_OSA_InterruptEnable
 * Description   : self explanatory.
 *
 *END**************************************************************************/
void FRM_OSA_InterruptEnable(void)
{
#if OsaBasePri_d
  if (__get_IPSR())
  {
...
#else
  FRM_OSA_EnableIRQGlobal();
#endif
}

/*FUNCTION**********************************************************************
 *
 * Function Name : FRM_OSA_InterruptDisable
 * Description   : self explanatory.
 *
 *END**************************************************************************/
void FRM_OSA_InterruptDisable(void)
{
#if OsaBasePri_d
  if (__get_IPSR())
  {
    if(g_base_priority_top < FRM_OSA_MAX_ISR_CRITICAL_SECTION_DEPTH)
    {
...
#else
  FRM_OSA_DisableIRQGlobal();
#endif
}

/*FUNCTION**********************************************************************
 *
 * Function Name : FRM_OSA_InterruptEnableRestricted
 * Description   : Disable interrupts except high-priority ones
 *
 *END**************************************************************************/
void FRM_OSA_InterruptEnableRestricted(uint32_t *pu8OldIntLevel)
{
...
/*FUNCTION**********************************************************************
 *
 * Function Name : FRM_OSA_InterruptEnableRestore
 * Description   : Restore interrupts that were previously restricted by a call
 *                 to FRM_OSA_InterruptEnableRestricted
 *
 *END**************************************************************************/
void FRM_OSA_InterruptEnableRestore(uint32_t *pu8OldIntLevel)
{
...
/*FUNCTION**********************************************************************
 *
 * Function Name : FRM_OSA_EnableIRQGlobal
 * Description   : enable interrupts using PRIMASK register.
 *
 *END**************************************************************************/
void FRM_OSA_EnableIRQGlobal(void)
{
...
/*FUNCTION**********************************************************************
 *
 * Function Name : FRM_OSA_DisableIRQGlobal
 * Description   : disable interrupts using PRIMASK register.
 *
 *END**************************************************************************/
void FRM_OSA_DisableIRQGlobal(void)
{
...
/*FUNCTION**********************************************************************
 *
 * Function Name : FRM_OSA_InstallIntHandler
 * Description   : This function is used to install interrupt handler.
 *
 *END**************************************************************************/
void FRM_OSA_InstallIntHandler(uint32_t IRQNumber, void (*handler)(void))
{
...
/*FUNCTION**********************************************************************
 *
 * Function Name : FRM_OSA_TimeInit
 * Description   : Empty
 *
 *END**************************************************************************/
void FRM_OSA_TimeInit(void)
{
...
********************************************************************************** */
#ifndef FSL_FRM_OSA_MAIN_FUNC_DISABLE
FRM_OSA_TASK_DEFINE(startup_task, gMainThreadPriority_c, 1, gMainThreadStackSize_c, 0)  ;
int main (void)
{
    /* Initialize MCU clock */
    hardware_init();
    FRM_OSA_TaskCreate(FRM_OSA_TASK(startup_task), NULL);
    vTaskStartScheduler();
