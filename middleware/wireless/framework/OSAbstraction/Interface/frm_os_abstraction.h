...
#ifndef _FRM_OS_ABSTRACTION_H_
#define _FRM_OS_ABSTRACTION_H_

#include "EmbeddedTypes.h"
#include "frm_os_abstraction_config.h"
...
/*! @brief Type for the timer definition*/    
  ...
  /*! @brief Type for a task handler, returned by the FRM_OSA_TaskCreate function. */  
...
/*! @brief Type for the semaphore handler, returned by the FRM_OSA_SemaphoreCreate function. */    
  typedef void* osaSemaphoreId_t; 
/*! @brief Type for the mutex handler, returned by the FRM_OSA_MutexCreate function. */      
  typedef void* osaMutexId_t; 
/*! @brief Type for the event handler, returned by the FRM_OSA_EventCreate function. */        
...
/*! @brief Type for the message queue handler, returned by the FRM_OSA_MsgQCreate function. */          
  typedef void* osaMsgQId_t; 
  /*! @brief Type for the Timer handler, returned by the FRM_OSA_TimerCreate function. */          
...
#define FRM_OSA_PRIORITY_IDLE          (6)
#define FRM_OSA_PRIORITY_LOW           (5)
#define FRM_OSA_PRIORITY_BELOW_NORMAL  (4)
#define FRM_OSA_PRIORITY_NORMAL        (3)
#define FRM_OSA_PRIORITY_ABOVE_NORMAL  (2)
#define FRM_OSA_PRIORITY_HIGH          (1)
#define FRM_OSA_PRIORITY_REAL_TIME     (0)
#define FRM_OSA_TASK_PRIORITY_MAX (0)
#define FRM_OSA_TASK_PRIORITY_MIN (15)  
 ...
 */ 
#if defined(FSL_RTOS_MQX)        
#define FRM_OSA_TASK_DEFINE(name, priority, instances, stackSz, useFloat)  \
 ...
 */
#define FRM_OSA_TASK(name)  \
&os_thread_def_##name

#define FRM_OSA_TASK_PROTO(name)  \
...
#define FRM_OSA_TIMER_DEF(name, function)  \
 ...
 * by the macro FRM_OSA_TASK_DEFINE.
 *
 ...
 * Example:
   @code
   osaTaskId_t taskId;
   FRM_OSA_TASK_DEFINE( Job1, FRM_OSA_PRIORITY_HIGH,  1,   800, 0);;
   taskId = FRM_OSA__TaskCreate(FRM_OSA__TASK(Job1), (osaTaskParam_t)NULL);
   @endcode
 */
 ...
 * @retval FALSE No task of higher priority than the current task got ready.
  */
bool_t FRM_OSA_TaskShouldYield(void);
#endif
 ...
 *
 * @param taskId The handler of the task to destroy. Returned by the FRM_OSA_TaskCreate function.
 *
 ...
 *
 * @param msgQId  pointer to queue returned by the FRM_OSA_MsgQCreate function.
 ...
 */
osaStatus_t FRM_OSA_MsgQPut(osaMsgQId_t msgQId, osaMsg_t pMessage);
 ...
 *
 * @param msgQId   Queue handler returned by the FRM_OSA_MsgQCreate function.
 ...
 */
osaStatus_t FRM_OSA_MsgQGet(osaMsgQId_t msgQId, osaMsg_t pMessage, uint32_t millisec);
...
*/
void FRM_OSA_InterruptEnableRestricted(uint32_t *pu32OldIntLevel);
...
*/
void FRM_OSA_InterruptEnableRestore(uint32_t *pu32OldIntLevel);
...
*/
void FRM_OSA_TimeResetMsec(void);
