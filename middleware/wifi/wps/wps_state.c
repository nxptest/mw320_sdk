...
static int (*wps_msg_prepare[18])(PWPS_INFO) = {
    ...
    wps_done_message_prepare,
    wps_eap_fail_frame_prepare,
    NULL
#endif
};
...
void wps_tls_init()
{
    ...
    /**
     * Setup key_material pointer in ssl configuration context
     */
    char key_material_label[25] = "client EAP encryption";
    mbedtls_ssl_conf_set_key_material(wps_s->wpa2_network.security.wlan_ctx, key_material_label, TLS_KEY_LEN,
                                      wps_s->key_data);
    ...
    if (wps_s->wpa2_network.security.type == WLAN_SECURITY_PEAP_MSCHAPV2)
    ...
    wpa2_ent_hexdump("EAP Session key", wps_s->key_data, PMK_LEN);

    wpa2_ent_d("Negotiated SSL protocol: %s", mbedtls_ssl_get_version(wps_s->wpa2_network.security.wlan_ssl));
    wpa2_ent_d("SSL cipher suite: %s", mbedtls_ssl_get_ciphersuite(wps_s->wpa2_network.security.wlan_ssl));
                                    ...
                                    u8 *master_key)
{
    ...
    wpa2_ent_hexdump("Challenge", challenge, 8);
    ...
    wpa2_ent_hexdump("password hash", pass_hash, 16);
    ...
    wpa2_ent_hexdump("Hash of password hash", pass_hash_hash, 16);
    ...
    wpa2_ent_hexdump("NT response", nt_response, MSCHAPV2_NT_RESPONSE_LEN);
    ...
    wpa2_ent_hexdump("auth response", auth_response, MSCHAPV2_AUTH_RESPONSE_LEN);
    ...
    wpa2_ent_hexdump("Master Key", master_key, MSCHAPV2_MASTER_KEY_LEN);
...
static int prepare_mschapv2_challenge_reply(const u8 id, const u8 mschapv2_identifier, const u8 *auth_challenge)
{
    ...
    wpa2_ent_d("EAP-MSCHAPv2: Generating challenge response");
    ...
    wpa2_ent_d("identity ascii: %s", id_str);
    wpa2_ent_hexdump("identity", id_str, id_len);
    username_len = id_len;
    username     = remove_mschapv2_domain(id_str, &username_len);
    wpa2_ent_d("username ascii: %s", username);
    wpa2_ent_hexdump("username", username, username_len);
    wpa2_ent_d("password ascii: %s", pass);
    wpa2_ent_hexdump("password", pass, pass_len);

    wps_generate_nonce_16B(wps_s->peer_challenge);

    wpa2_ent_hexdump("auth challenge", auth_challenge, MSCHAPV2_CHAL_LEN);

    wpa2_ent_hexdump("peer challenge", wps_s->peer_challenge, MSCHAPV2_CHAL_LEN);
    ...
    memcpy((u8 *)peap_frame + sizeof(EAP_FRAME_HEADER2), &eap_type, sizeof(u8));
    ...
    memcpy((u8 *)mschapv2_frame + sizeof(EAP_MSCHAPV2_FRAME_HEADER), &resp_len, sizeof(u8));
    ...
    memcpy(mschapv2_resp->peer_challenge, wps_s->peer_challenge, MSCHAPV2_CHAL_LEN);

    memset(mschapv2_resp->reserved, 0, 8);

    memcpy(mschapv2_resp->nt_response, wps_s->nt_response, MSCHAPV2_NT_RESPONSE_LEN);

    mschapv2_resp->flags = 0;

    memcpy((u8 *)mschapv2_resp + sizeof(EAP_MSCHAPV2_RESPONSE_HEADER), id_str, id_len);

    wpa2_ent_hexdump("Challenge Response", (u8 *)pwps_info->buffer, (size_t)length);
...
static int process_mschapv2_challenge(const u8 id, PEAP_MSCHAPV2_FRAME_HEADER ms, size_t ms_len)
{
    ...
    wpa2_ent_d("EAP-MSCHAPv2: Processing challenge");
    ...
    wpa2_ent_hexdump("Authentication servername", p, len);
...
static int verify_mschapv2_auth_response(const u8 id, PEAP_MSCHAPV2_FRAME_HEADER ms, size_t ms_len)
{
    ...
    memcpy((u8 *)peap_frame + sizeof(EAP_FRAME_HEADER2) + 1, &success, sizeof(u8));

    wpa2_ent_hexdump("Auth Response", (u8 *)pwps_info->buffer, (size_t)length);
...
static int process_mschapv2_message(const u8 *msg)
{
    ...
    wpa2_ent_d("EAP-MSCHAPv2: id: %d Request code: %d", hdr->identifier, ms->code);
    ...
    switch (ms->code)
    {
        ...
        case MSCHAPV2_SUCCESS:
            ...
            wpa2_ent_d("Received Success");
            wpa2_ent_d("Authentication succeeded");
            break;
...
static int peap_prepare_tlv_result(const u8 *msg, int resp_status)
{
    ...
    {
        memcpy((u8 *)peap_frame + sizeof(EAP_FRAME_HEADER2) + 1, success_tlv, 6);
    }
    else
    {
        memcpy((u8 *)peap_frame + sizeof(EAP_FRAME_HEADER2) + 1, failure_tlv, 6);
    }

    wpa2_ent_hexdump("TLV Response", (u8 *)pwps_info->buffer, (size_t)length);
...
static int peap_process_tlv(const u8 *msg)
{
    ...
    if (tlv[0] == 0x80 && tlv[1] == 0x03)
    {
        ...
        {
            wpa2_failure = 1;
            wpa2_ent_d("EAP-TLV: TLV Result - Failure");
            resp_status = EAP_TLV_RESULT_FAILURE;
        }
        else
        {
            wpa2_failure = 1;
            wpa2_ent_d("EAP-TLV: Unknown TLV Result %d", status);
            resp_status = EAP_TLV_RESULT_FAILURE;
        }
    }
    else
...
static int process_peap_message(const u8 *msg)
{
    ...
    wpa2_ent_d("EAP-PEAP: Received Phase 2: code %d, id %d, len %d", hdr->code, hdr->identifier, ntohs(hdr->length));
    ...
    else if (peap->type == EAP_TYPE_TLS || peap->type == EAP_TYPE_PEAP)
    {
        ...
        else if (rbuf)
        {
            ...
            if (wps_s->tls_session_active)
            {
                ...
                if (msg_length)
                {
                    memcpy(rbuf, (u8 *)peap + sizeof(EAP_TLS_FRAME_HEADER), msg_length);

                    rlen = msg_length;

                    wpa2_ent_d("Tunnel EAP Request id: %d", peap_tls->identifier);
                    wpa2_ent_hexdump("Encrypted Data", rbuf, rlen);

                    plain_data_len =
                        wm_mbedtls_ssl_read(wps_s->wpa2_network.security.wlan_ssl, plain_data, plain_data_max_len);

                    wpa2_ent_hexdump("Plain Data", plain_data, plain_data_len);
                    ...
                    if (!skip_change)
                    {
                        ...
                        memcpy(msg + sizeof(EAP_FRAME_HEADER2), plain_data, plain_data_len);
            ...
            else
            {
                ...
                if ((peap_tls->flags & EAP_TLS_FLAGS_MORE_FRAGMENTS) && (!fragment))
                {
                    ...
                    if (msg_length)
                    {
                        if (peap_tls->flags & EAP_TLS_FLAGS_LENGTH_INCLUDED)
                        {
                            ...
                            memcpy(rbuf, (u8 *)peap + sizeof(EAP_TLS_FRAME_HEADER) + sizeof(u32), msg_length);
                        }
                        else
                            memcpy(rbuf, (u8 *)peap + sizeof(EAP_TLS_FRAME_HEADER), msg_length);
                ...
                else if ((peap_tls->flags & EAP_TLS_FLAGS_LENGTH_INCLUDED) && fragment)
                {
                    ...
                    if (msg_length)
                    {
                        memcpy(rbuf + used_len + rlen, (u8 *)peap + sizeof(EAP_TLS_FRAME_HEADER) + sizeof(u32),
                               msg_length);
                        rlen += msg_length;
                ...
                else if (fragment)
                {
                    ...
                    if (msg_length)
                    {
                        memcpy(rbuf + used_len + rlen, (u8 *)peap + sizeof(EAP_TLS_FRAME_HEADER), msg_length);
            ...
            switch (peap->code)
            {
                ...
                case EAP_FAILURE:
                {
                    ...
                    if (pwps_info->state == WPS_STATE_A && pwps_info->register_completed == WPS_SET)
                    {
                        ...
                        if (pwps_info->role == WPS_REGISTRAR)
                        {
                            memset(&pwps_info->enrollee.mac_address, 0, ETH_ALEN);
                        }
                        ...
                        memset(&p2p_network, 0, sizeof(struct wlan_network));
                        memcpy(&p2p_network, &network, sizeof(struct wlan_network));
...
#else
                        if (wps.cb(WPS_SESSION_SUCCESSFUL, &network, sizeof(struct wlan_network)) == -WM_FAIL)
...
static int retransmit_last_tx_pkt(PWPS_INFO pwps_info)
{
    ...
    if (pwps_info->eap_msg_sent != WPS_EAPOL_MSG_START)
    {
        ...
        wps_start_timer(WPS_MSG_DEFAULT_TIMEOUT / 1000, 0, wps_txTimer_handler, pwps_info);
        pwps_info->set_timer = WPS_SET;
        pwps_info->wps_msg_resent_count++;
#ifndef CONFIG_WPA2_ENTP
    }
    ...
    {
        ret = wps_msg_transmit(pwps_info, WPS_EAPOL_MSG_START, WPS_MSG_DEFAULT_TIMEOUT);
    }
#endif /*ifndef CONFIG_WPA2_ENTP*/
...
void wps_txTimer_handler(void *user_data)
{
    ...
    if (wps_s->bss_type == BSS_TYPE_STA)
    {
        ...
        if (pwps_info->restart_link_lost || pwps_info->restart_by_M2D)
        {
            ...
            status                          = wps_msg_transmit(pwps_info, WPS_EAPOL_MSG_START, WPS_MSG_DEFAULT_TIMEOUT);
...
int wps_state_machine_start(PWPS_INFO pwps_info)
{
    ...
    if (((pwps_info->role == WPS_ENROLLEE) && wps_s->bss_type == BSS_TYPE_STA) || (IS_DISCOVERY_ENROLLEE(pwps_info)))
    {
        /*
...
void wps_reset_wps_state(WPS_INFO *pwps_info)
{
    ...
    {
        memset(pwps_info->last_message.message, 0, pwps_info->last_message.length);
    }
