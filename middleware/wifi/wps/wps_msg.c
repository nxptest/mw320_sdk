...
static int wps_eap_message_header_prepare(PWPS_INFO wps_info, u8 code, u8 id, u8 wps_msg)
{
    ...
    memcpy(peap_wps_frame->vendor_type, wps_vendor_type, sizeof(wps_vendor_type));
...
static int wps_subele_authorized_mac_prepare(PWPS_INFO pwps_info, u8 *ptr)
{
    ...
    while (auth_enrollee_count < MAX_AUTH_ENROLLEE_COUNT)
    {
        ...
        {
            memcpy(ptr, &pwps_info->auth_enrollee_mac_addr[auth_enrollee_count * ETH_ALEN], ETH_ALEN);
...
static int wps_attribute_public_key_prepare(PWPS_INFO pwps_info, u8 type, u8 *ptr)
{
    ...
    if (type == WPS_ENROLLEE)
    {
        ...
        pwps_info->dh = mrvl_dh_setup_key(pwps_info->enrollee.public_key, SZ_PUBLIC_KEY,
    ...
    else if (type == WPS_REGISTRAR)
    {
        ...
        /* free the old DH first before starting the new DH */
        mrvl_dh_free(pwps_info->dh);
        pwps_info->dh = mrvl_dh_setup_key(pwps_info->registrar.public_key, SZ_PUBLIC_KEY,
    ...
    PWPS_INFO pwps_info, u8 *prev_msg, u16 prev_msg_len, u8 *curr_msg, u16 curr_msg_len, u8 *hmac_out)
{
    ...
    mrvl_hmac_sha256(pwps_info->auth_key, SZ_AUTH_KEY, hmacIn, hmacLen, hmac_out, SHA256_DIGEST_SIZE);
...
int output_supp_config(PWPS_INFO pwps_info, int index, int nCred)
{
        ...
        pCredential->auth_type == AUTHENTICATION_TYPE_WPA2PSK)
    {
        ...
        memcpy(network.name, pCredential->ssid, pCredential->ssid_length);
        memcpy(network.ssid, pCredential->ssid, pCredential->ssid_length);
#ifdef CONFIG_P2P
        network.channel = g_channel;
#endif
        ...
        network.security.psk_len = pCredential->network_key_len;
        memcpy(network.security.psk, pCredential->network_key, pCredential->network_key_len);
    ...
    else if (pCredential->auth_type == AUTHENTICATION_TYPE_OPEN || pCredential->auth_type == AUTHENTICATION_TYPE_SHARED)
    {
        ...
        memcpy(network.name, pCredential->ssid, pCredential->ssid_length);
        memcpy(network.ssid, pCredential->ssid, pCredential->ssid_length);
        memcpy(network.bssid, wps_network.bssid, IEEEtypes_ADDRESS_SIZE);
...
int wps_peap_response_identity_prepare(PWPS_INFO pwps_info)
{
    ...
    memcpy((u8 *)peap_frame + sizeof(EAP_FRAME_HEADER2) + 1, id_str, strlen(id_str));
...
#ifdef CONFIG_FREE_RADIUS
        if (len > 600 || (flag && more_frags < 4))
        {
            flag = true;
            more_frags++;
            peap_tls_frame->flags = 0x40;
        }
        else
#endif
            peap_tls_frame->flags = 0x00;
        peap_tls_frame->length = 6;
...
int wps_eap_response_identity_process(PWPS_INFO pwps_info, u8 *buf, u16 size)
{
    ...
    if (memcmp(buf, wps_registrar_identity, strlen((char *)wps_registrar_identity)) == 0)
    {
        ...
        if ((wps_s->bss_type == BSS_TYPE_UAP && pwps_info->role == WPS_REGISTRAR))
        {
            ...
            memcpy(pwps_info->registrar.mac_address, pwps_info->enrollee.mac_address, ETH_ALEN);
    ...
    else if (memcmp(buf, wps_enrollee_identity, strlen((char *)wps_enrollee_identity)) == 0)
    {
        ...
        if ((wps_s->bss_type == BSS_TYPE_UAP && pwps_info->role == WPS_ENROLLEE))
        {
            ...
            memcpy(pwps_info->enrollee.mac_address, pwps_info->registrar.mac_address, ETH_ALEN);
...
int wps_eap_M1_frame_process(PWPS_INFO pwps_info, u8 *buf, u16 size)
{
    ...
    memcpy(&pwps_info->last_tx_message, &pwps_info->last_message, sizeof(MESSAGE_BUFFER));
...
static int wps_derive_kdk(PWPS_INFO pwps_info)
{
    ...
    shared_key_len = mrvl_dh_compute_key(pwps_info->dh, agreed_key, WPS_AGREE_KEY_LEN, pub_key, WPS_PUB_KEY_LEN,
    ...
    if (!mrvl_sha256_vector(1, msg, &AgreedKeyLen, DHKey, sizeof(DHKey)))
    {
        wps_d("SHA256 failed!\n");
        return WPS_STATUS_FAIL;
    }
    ...
    memcpy(HashKdkIn + 16, pwps_info->enrollee.mac_address, sizeof(pwps_info->enrollee.mac_address));
    memcpy(HashKdkIn + 16 + 6, pwps_info->registrar.nonce, sizeof(pwps_info->registrar.nonce));

    /* Generate KDK Key */
    memset(KDK, 0, sizeof(KDK));
    mrvl_hmac_sha256(DHKey, SHA256_DIGEST_SIZE, HashKdkIn, sizeof(HashKdkIn), KDK, SHA256_DIGEST_SIZE);

    /* Generate Authentication Key, Key Wrap Key */
    memset(AKE_Key, 0, sizeof(AKE_Key));
    mrvl_kdf(KDK, SHA256_DIGEST_SIZE, AKE_Key, sizeof(AKE_Key));
    ...
    while ((void *)ptlv < (void *)plast_byte)
    {
        ...
        switch (tlv_type)
        {
            ...
            case SC_Encryption_Type_Flags:
                ...
                break;
            case SC_Connection_Type_Flags:
                memcpy(&pwps_info->registrar.connection_type_flag, data, tlv_length);
                break;
            case SC_Config_Methods:
                memcpy(&data16, data, tlv_length);
...
int wps_eap_M3_frame_prepare(PWPS_INFO pwps_info)
{
    ...
    mrvl_hmac_sha256(pwps_info->auth_key, SZ_AUTH_KEY, pwps_info->PIN, pwps_info->PINLen / 2, RawPSK,
                     SHA256_DIGEST_SIZE);
    ...
    mrvl_hmac_sha256(pwps_info->auth_key, SZ_AUTH_KEY, &pwps_info->PIN[pwps_info->PINLen / 2], pwps_info->PINLen / 2,
    ...
    offset += WPS_PUB_KEY_LEN;
    mrvl_hmac_sha256(pwps_info->auth_key, SZ_AUTH_KEY, HashArgu, offset, HASH, SHA256_DIGEST_SIZE);
...
int wps_eap_M4_frame_prepare(PWPS_INFO pwps_info)
{
    ...
    mrvl_hmac_sha256(pwps_info->auth_key, SZ_AUTH_KEY, EncryData, 20, KWA, SHA256_DIGEST_SIZE);
    ...
    mrvl_aes_wrap(EncryData, pEncry - EncryData, EncryAesOutput, pwps_info->key_wrap_key, 16, pwps_info->registrar.IV);
    ...
    while ((void *)ptlv < (void *)plast_byte)
    {
        ...
        switch (tlv_type)
        {
            ...
            case SC_Encrypted_Settings:
            {
                ...
                memcpy(pwps_info->registrar.encrypted_data, data + 16, pwps_info->registrar.encrypted_data_len);
                PTLV_DATA_HEADER pes = (PTLV_DATA_HEADER)(pwps_info->registrar.wrap_raw_data);
                memset(pwps_info->registrar.wrap_raw_data, 0x00, sizeof(pwps_info->registrar.wrap_raw_data));
                mrvl_aes_unwrap(pwps_info->registrar.encrypted_data, pwps_info->registrar.encrypted_data_len,
...
int wps_eap_M5_frame_prepare(PWPS_INFO pwps_info)
{
    ...
    ptlvE->length = htons(ptlvE->length);
    memset(EncryAesOutput, 0, sizeof(EncryAesOutput));
    mrvl_aes_wrap(EncryData, pEncry - EncryData, EncryAesOutput, pwps_info->key_wrap_key, 16, pwps_info->enrollee.IV);
    ...
    while ((void *)ptlv < (void *)plast_byte)
    {
        ...
        switch (tlv_type)
        {
            ...
            case SC_Encrypted_Settings:
            {
                ...
                memcpy(pwps_info->enrollee.encrypted_data, data + 16, pwps_info->enrollee.encrypted_data_len);

                PTLV_DATA_HEADER pes = (PTLV_DATA_HEADER)(pwps_info->enrollee.wrap_raw_data);
                memset(pwps_info->enrollee.wrap_raw_data, 0x00, sizeof(pwps_info->enrollee.wrap_raw_data));
                mrvl_aes_unwrap(pwps_info->enrollee.encrypted_data, pwps_info->enrollee.encrypted_data_len,
...
int wps_add_ap_settings_to_registrar(PWPS_INFO pwps_info, u8 *pEncry)
{
    ...
    pEncry += SZ_TLV_HEADER;
    memcpy(pEncry, pwps_info->enrollee.cred_data[0].network_key,
           ptlvC->length);
    pEncry += ptlvC->length;
    ap_setting_len += (SZ_TLV_HEADER + ptlvC->length);
    ptlvC->length = htons(ptlvC->length);
...
int wps_eap_M7_frame_prepare(PWPS_INFO pwps_info)
{
    ...
    /* Key Wrap Authenticator */
    memset(KWA, 0, sizeof(KWA));
    mrvl_hmac_sha256(pwps_info->auth_key, SZ_AUTH_KEY, EncryData, pEncry - EncryData, KWA, SHA256_DIGEST_SIZE);
    ...
    enc_setting_len = mrvl_aes_wrap_ext(EncryData, pEncry - EncryData, EncryAesOutput, pwps_info->key_wrap_key, 16,
...
int wps_eap_M8_frame_prepare(PWPS_INFO pwps_info)
{
    ...
    *pEncry = 0x1;
    ...
    pEncry += SZ_TLV_HEADER;
    memcpy(pEncry, pwps_info->registrar.cred_data[0].network_key, ptlvC->length);
    pEncry += ptlvC->length;
    enc_setting_len += (SZ_TLV_HEADER + ptlvC->length);
    ptlvC->length = htons(ptlvC->length);
    ...
    if (datalength % 32 == 0)
    {
        ...
        memset(EncryAesOutput, 0, sizeof(EncryAesOutput));
        mrvl_aes_wrap(EncryData, datalength, EncryAesOutput, pwps_info->key_wrap_key, 16, pwps_info->registrar.IV);
    ...
    while ((void *)ptlv < (void *)plast_byte)
    {
        ...
        switch (tlv_type)
        {
            ...
            case SC_Encrypted_Settings:
            {
                ...
                num_cred = 0;
                memset(pwps_info->registrar.cred_data, 0x00, MAX_NUM_CRDENTIAL * sizeof(CREDENTIAL_DATA));
                ...
                pwps_info->registrar.encrypted_data_len = mrvl_aes_unwrap_ext(
                        ...
                        case SC_Credential:
                        {
                            ...
                            while (((u8 *)pcre - esdata) < pes->length)
                            {
                                ...
                                switch (cretype)
                                {
                                    case SC_Network_Index:
                                        wps_d("SC_Network_Index");
                                        memcpy(&pCred->network_index, credata, pcre->length);
                                        break;
                                    case SC_SSID:
                                        wps_d("SC_SSID");
                                        pCred->ssid_length = MIN(pcre->length, sizeof(pCred->ssid));
                                        memcpy(pCred->ssid, credata, pCred->ssid_length);
                                        wps_d("SSID : %s", pCred->ssid);
                                        wps_d("SSID length: %d", pCred->ssid_length);
                                        break;
                                    case SC_Authentication_Type:
                                        wps_d("SC_Authentication_Type");
                                        memcpy(&data16, credata, pcre->length);
                                    ...
                                    case SC_Network_Key_Index:
                                        wps_d("SC_Network_Key_Index");
                                        memcpy(&pCred->network_key_index, credata, pcre->length);
                                        break;
                                    case SC_WEP_Transmit_Key:
                                        wps_d("SC_WEP_Transmit_Key");
                                        memcpy(&pCred->wep_tx_key_index, credata, pcre->length);
                                        break;
                                    case SC_Network_Key:
                                        wps_d("SC_Network_Key");
                                        wps_hexdump("SC_Network_Key", (u8 *)credata, pcre->length);
                                        memcpy(pCred->network_key, credata, pcre->length);
                                        pCred->network_key_len = pcre->length;
                                        break;
                                    case SC_MAC_Address:
                                        wps_d("SC_MAC_Address");
                                        memcpy(pCred->mac_address, credata, pcre->length);
                                        break;
                                    case SC_RF_Band:
                                        wps_d("SC_RF_Band");
                                        memcpy(&pCred->rf_bands, credata, pcre->length);
                                        break;
                                }
...
int wps_done_message_process(PWPS_INFO pwps_info, u8 *buf, u16 size)
{
     ...
     * Save received frame for Authenticator
     */
    memcpy(pwps_info->last_message.message, (u8 *)(buf + SZ_EAP_WPS_FRAME_HEADER), (size - SZ_EAP_WPS_FRAME_HEADER));
...
int wps_ap_assoc_response_prepare(u8 selectedRegistrar, u8 *ptr)
{
    ...
    if (en_reg->version >= WPS_VERSION_2DOT0) {
        ...
        if (selectedRegistrar == 1) {
            /* Authorized MAC sub-ele */
            offset =
                wps_subele_authorized_mac_prepare(pwps_info, ptr);
            ptr += offset;
        }
        /* Set Vendor Extension Attr len */
        wfa_ven_tlv->length =
