...
struct wlan_network network PM3_BSS;
#ifdef CONFIG_P2P
...
static os_queue_pool_define(wps_cmd_queue_data, sizeof(struct wps_command **) * MAX_COMMANDS);
...
#ifdef CONFIG_WPA2_ENTP
static os_thread_stack_define(wps_stack, 2048);
#else
...
void wps_generate_uuid_using_mac_addr(const u8 *mac_addr, u8 *uuid)
{
    ...
    addr[1] = mac_addr;
    len[1]  = 6;
    ret     = mrvl_sha1_vector(2, addr, len, hash, sizeof(hash));
...
static int wps_parameter_initialization(PWPS_INFO pwps_info, WPS_DATA *wps_s)
{
    ...
    if (!(memcmp(pwps_info->enrollee.wps_uuid, zero_uuid, WPS_UUID_MAX_LEN)))
    {
        ...
        memcpy(&pwps_info->enrollee.wps_uuid, &pwps_info->registrar.wps_uuid, WPS_UUID_MAX_LEN);
...
int wps_connect(enum wps_session_command pbc, uint32_t pin, struct wlan_scan_result *res)
{
    ...
    wps_cmd = (struct wps_command *)wps_mem_malloc(sizeof(struct wps_command));

    if (wps_cmd == NULL)
    {
        PRINTF("Failed to allocated memory for wps command\r\n");
        return -WM_FAIL;
    }
...
int wpa2_ent_connect(struct wlan_network *wpa2_network)
{
    ...
    wpa2_cmd = (struct wpa2_command *)wps_mem_malloc(sizeof(struct wpa2_command));

    if (wpa2_cmd == NULL)
    {
        PRINTF("Failed to allocated memory for wpa2 command\r\n");
        return -WM_FAIL;
    }

    memset(wpa2_cmd, 0, sizeof(struct wpa2_command));

    memcpy(&wpa2_cmd->wpa2_network, wpa2_network, sizeof(struct wlan_network));
...
static void wps_main(os_thread_arg_t data)
{
    ...
    while (1)
    {
        ...
        ret = os_queue_recv(&wps.cmd_queue, &wps_cmd, os_msec_to_ticks(1000));
        ...
        memcpy(&wps_s->wpa2_network, &wpa2_cmd->wpa2_network, sizeof(struct wlan_network));
        wps_mem_free(wpa2_cmd);
...
int wps_start(struct wps_config *wps_conf)
{
    ...
    ret = os_queue_create(&wps.cmd_queue, "wps_cmd_queue", sizeof(struct wps_command **), &wps.cmd_queue_data);
    if (ret != WM_SUCCESS)
    {
    ...
    memset(wps_s->ifname, 0, IFNAMESIZE + 1);
...
#else
#ifdef CONFIG_WPA2_ENTP
    ret             = os_thread_create(&wps_main_thread, "wpa2_ent", wps_main, 0, &wps_stack, OS_PRIO_2);
    if (ret != WM_SUCCESS)
    {
...
#else
    ret = os_thread_create(&wps_main_thread, "wps", wps_main, 0, &wps_stack, OS_PRIO_3);
    if (ret != WM_SUCCESS)
    {
        wps_d("Error: Failed to create wps thread: %d\r\n", ret);
        goto fail;
