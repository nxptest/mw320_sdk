...
extern mlan_status wrapper_wlan_cmd_mgmt_ie(int bss_type, void *buffer, unsigned int len, unsigned int action);
...
int load_cred_info(WPS_DATA *wps_s, PWPS_INFO pwps_info, bss_config_t *bss)
{
    ...
    if (bss->ssid.ssid_len)
    {
        ...
        memcpy(p2p_uap_network.ssid, bss->ssid.ssid, bss->ssid.ssid_len);
    ...
    if (reg_cred->encry_type == ENCRYPTION_TYPE_WEP)
    {
        ...
        for (i = 0; i < 4; i++)
        {
            if (bss->wep_cfg[i].is_default)
            {
                ...
                memcpy(reg_cred->network_key, bss->wep_cfg[i].key, bss->wep_cfg[i].length);
        ...
        (reg_cred->auth_type == AUTHENTICATION_TYPE_WPA_MIXED) || (reg_cred->auth_type == AUTHENTICATION_TYPE_ALL))
    {
        ...
        memcpy(reg_cred->network_key, bss->wpa_cfg.passphrase, bss->wpa_cfg.length);
        if (reg_cred->encry_type != ENCRYPTION_TYPE_WEP)
        {
        ...
        switch (tlv_type)
        {
            case MRVL_SSID_TLV_ID:
                ...
                if (tlv_len)
                {
                    ...
                    memcpy(bss->ssid.ssid, ssid_tlv->ssid, bss->ssid.ssid_len);
                }
                break;
            ...
            case MRVL_WPA_PASSPHRASE_TLV_ID:
                ...
                if (bss->wpa_cfg.length)
                    memcpy(bss->wpa_cfg.passphrase, psk_tlv->passphrase, bss->wpa_cfg.length);
                break;
            default:
                break;
        }
...
int wps_wlan_ie_config(int flag, short *pie_index)
{
    ...
    if (action == SET_WPS_IE)
    {
       ...
       wps_ap_assoc_response_prepare(WPS_END_REG_DISCOVERY_PHASE, pos);*/
        ...
        tlv->length               = sizeof(custom_ie) + sizeof(IEEEtypes_Header_t) + ie_len - MAX_IE_SIZE + 1;
        ...
        {
            ie_ptr      = (custom_ie *)(((u8 *)ie_ptr) + sizeof(custom_ie) + sizeof(IEEEtypes_Header_t) + ie_len -
                                   MAX_IE_SIZE + 1);
            pos         = ie_ptr->ie_buffer;
            ptlv_header = (IEEEtypes_Header_t *)pos;
            pos += sizeof(IEEEtypes_Header_t);
    ...
    else if (action == CLEAR_WPS_IE)
    {
        ...
        {
            ie_ptr                    = (custom_ie *)(((u8 *)ie_ptr) + sizeof(custom_ie) - MAX_IE_SIZE + 1);
            ie_ptr->mgmt_subtype_mask = MGMT_MASK_CLEAR;
            ie_ptr->ie_length         = 0;
            ie_ptr->ie_index          = *pie_index + 1;
            tlv->length               = 2 * (sizeof(custom_ie) - MAX_IE_SIZE + 1);
        }
        else
            tlv->length = sizeof(custom_ie) - MAX_IE_SIZE + 1;
...
void wlan_generate_psk(PWPS_INFO pwps_info)
{
    ...
    memcpy(buf + sizeof(tlvbuf_custom_ie), (u8 *)pwps_info->registrar.cred_data[0].network_key, 64);
