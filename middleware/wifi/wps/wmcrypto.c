...
int mrvl_dh_compute_key(void *dh,
                        uint8_t *shared_key,
                        uint32_t shared_len,
                        uint8_t *public_key,
                        uint32_t public_len,
                        ...
                        DH_PG_PARAMS *dh_params)
{
     ...
     * in dh context by function 'mrvl_dh_setup_key'
     */
...
uint32_t mrvl_sha1_vector(size_t nmsg, const uint8_t *msg[], const size_t msglen[], uint8_t *mac, size_t maclen)
{
    if (!is_wm_mbedtls_lib_init())
    {
        crypto_e("mbedtls lib init not done");
...
static uint32_t mrvl_md5_vector(size_t nmsg, const uint8_t *msg[], const size_t msglen[], uint8_t *mac, size_t maclen)
{
    md5_vector(nmsg, msg, msglen, mac);
    return 0;
}

uint32_t mrvl_sha256_vector(size_t nmsg, const uint8_t *msg[], const size_t msglen[], uint8_t *mac, size_t maclen)
{
...
void mrvl_sha256(size_t num_elem, const uint8_t *addr[], const size_t *len, uint8_t *mac)
{
    ...
    const uint8_t *key, uint32_t keylen, uint8_t *msg, uint32_t msglen, uint8_t *mac, uint32_t maclen)
{
    ...
    if (keylen > 64)
    {
        mrvl_sha256_vector(1, &key, (size_t *)&keylen, tk, SHA256_DIGEST_SIZE);
        key    = tk;
 ...
 */
int mrvl_kdf(uint8_t *key, uint32_t key_len, uint8_t *result, uint32_t result_len)
{
 ...
 */
int mrvl_aes_wrap(uint8_t *pPlainTxt, uint32_t TextLen, uint8_t *pCipTxt, uint8_t *pKEK, uint32_t KeyLen, uint8_t *IV)
{
 ...
 *  @return             0 on success, -1 on failure
 */
int mrvl_aes_unwrap(uint8_t *pCipTxt, uint32_t TextLen, uint8_t *pPlainTxt, uint8_t *pKEK, uint32_t KeyLen, uint8_t *IV)
{
    //    aes_t dec;
...
void mrvl_crypto_hmac_md5(uint8_t *input, int input_len, uint8_t *hash, char *hash_key)
{
 ...
 */
static int mrvl_passwordhash(const char *passphrase, const char *ssid, int ssid_len, char *output)
{
...
int mrvl_generate_psk(const char *ssid, int ssid_len, const char *passphrase, char *output)
{
    int i, ret;
    ...
    if (output && passphrase)
    {
        ...
        {
            ret = mrvl_passwordhash(passphrase, ssid, ssid_len, output);
            if (ret != 0)
                return ret;
        }
        else if (i == PSK_PASS_PHRASE_LEN_MAX)
        {
            ...
            {
                output[i / 2] = ((mrvl_get_hex_nibble(passphrase[i]) << 4) | mrvl_get_hex_nibble(passphrase[i + 1]));
            }
        }
    }
 ...
 */
int mrvl_tls_prf(const uint8_t *secret,
                 size_t secret_len,
                 const char *label,
                 const uint8_t *seed,
                 size_t seed_len,
 ...
 */
int mrvl_sha1_prf(const uint8_t *key,
                  size_t key_len,
                  const char *label,
                  const uint8_t *data,
                  size_t data_len,
