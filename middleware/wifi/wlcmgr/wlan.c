...
#ifdef CONFIG_WPS2
static os_thread_stack_define(g_wps_stack, 4096);
...
/* MW320 Specific Code Starts here */
    ...
    wlan_ed_mac_ctrl.ed_offset_2g = 0x0F;
...
static int prov_wps_scan_results(unsigned int count)
{
    ...
    for (i = 0; i < count; i++)
    {
        ...
        if ((wlan_wps.wps_res.wps == true) && ((wlan_wps.wps_res.wps_session == WPS_SESSION_PBC) || (wlan_wps.wps_res.wps_session == WPS_SESSION_PIN)))
            break;
    }
...
static void wps_main(os_thread_arg_t data)
{
    ...
    while (1)
    {
	    ...
	    (void)PRINTF("WPS is enabled\r\n");
            while (i)
            {
                wlan_scan(prov_wps_scan_results);
                os_semaphore_get(&wlan_wps.wps_scan_done, OS_WAIT_FOREVER);
                if ((wlan_wps.wps_res.wps == true) && ((wlan_wps.wps_res.wps_session == WPS_SESSION_PIN) ||
...
static int wlcm_wps_callback(enum wps_event event, void *data, uint16_t len)
{
    ...
    if (event == WPS_SESSION_SUCCESSFUL)
    {
        if (data == NULL || len != sizeof(struct wlan_network))
        {
            ...
            CONNECTION_EVENT(WLAN_REASON_WPS_UNSUCCESSFUL, NULL);
            return -WM_FAIL;
        }

        CONNECTION_EVENT(WLAN_REASON_WPS_SUCCESSFUL, (void *)net);
    }
