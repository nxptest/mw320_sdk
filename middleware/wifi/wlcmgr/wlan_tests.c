...
#ifdef SD8801
static void test_wlan_enable_ext_coex(int argc, char **argv)
{
    int ret = -WM_FAIL;
    ...
    ext_coex_config.EXT_RADIO_REQ_ip_gpio_num      = 0;
    ext_coex_config.EXT_RADIO_REQ_ip_gpio_polarity = 1;
    ext_coex_config.EXT_RADIO_PRI_ip_gpio_num      = 1;
    ext_coex_config.EXT_RADIO_PRI_ip_gpio_polarity = 1;
    ext_coex_config.WLAN_GRANT_op_gpio_num         = 10;
    ...
    if (ret == WM_SUCCESS)
    {
        (void)PRINTF("External Coex Config set successfully");
    }
    else
    {
        (void)PRINTF("External Coex Config error: %d", ret);
    }
}
static void test_wlan_ext_coex_stats(int argc, char **argv)
{
    int ret = -WM_FAIL;
...
static void test_wlan_start_wps_pin(int argc, char **argv)
{
    ...
    pin = atoi(argv[1]);
    PRINTF("Start WPS PIN session with %d pin\r\n", pin);
...
#ifdef SD8801
    {"wlan-enable-ext-coex", NULL, test_wlan_enable_ext_coex},
    {"wlan-get-ext-coex-stats", NULL, test_wlan_ext_coex_stats},
#endif
#ifdef CONFIG_WPS2
